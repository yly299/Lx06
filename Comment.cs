   ...
   public class Comment
    {
        ...
        public IEnumerable<Model.Comment> GetWorkPage(Model.CommentPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by commentTime desc) as num, Comment.*,workName from Comment where Comment.workId=@workId)";
                sql += "select * from a where num between (@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.CommentNo>(sql, page);
            }
        }
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment(commentID,workUd,userName,CommentContent,CommentTime)"+
                    "values(@commentID,@workUd,@userName,@CommentContent,@CommentTime)";
                sql += "SELECT @@IDENTITY";
